...
""")
    ...
    # src.parser -> src.base
    edge_to_base = next(e for e in edges if e["source"] == "src.parser" and e["target"] == "src.base")
    assert "BaseParser" in edge_to_base["data"]["interactions"][0]["imported_entities"]
    
    # src.parser -> src.logger
    edge_to_logger = next(e for e in edges if e["source"] == "src.parser" and e["target"] == "src.logger")
