...
## 🔭 愿景：多粒度透视 (Multi-Granularity X-Ray)

Archimedes 正在演进以解决上下文“一刀切 (One-size-fits-all)”的问题。提供整个代码库的骨架非常适合深度重构，但对于高层次的架构级提问来说显得过于庞大（导致不必要的 Token 浪费）。

为了最大化 Token 投资回报率 (ROI)，我们的架构采用了 **“多粒度”变焦镜头** 策略：
-   **L1: 系统/基础设施层 (Macro 广角)**：解析 `docker-compose.yml`（以及未来的 K8s 配置），提供微服务、端口和外部依赖的全局视角。*(最高优先级)*
-   **L2: 模块依赖层 (Mid-Macro 标准)**：当前的 `rustworkx` 导入图谱，映射文件级别的依赖关系。
-   **L3: 接口骨架层 (Mid-Micro 微距)**：当前基于 AST 提取的类和函数签名。
-   **L4: 深度调用层 (Micro 显微镜)**：函数级内部调用链追踪。*(最低优先级：由于工程 ROI 极低，这部分最适合交由大模型通过原生阅读目标源码文件来处理).*

通过优先发展 **L1 (系统层)**，Archimedes 将允许 AI Agent 在深入 Python 具体模块之前，瞬间建立起项目的宏观边界认知。

## 🗺 路线图

-   **V2.1**：高级边解析（将导入匹配到具体的函数/类）。
-   **V2.2**：L1 基础设施感知 — 增加对 `docker-compose.yml` 等配置的解析支持，提取顶层系统架构和服务依赖。
